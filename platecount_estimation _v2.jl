...
begin
	rel_vol_head = vapor(kelvin(t_head_c), Antoine["nheptane"]) / vapor(kelvin(t_head_c), Antoine["isooctane"])
	
	rel_vol_pot = vapor(kelvin(t_pot_c), Antoine["nheptane"]) / vapor(kelvin(t_pot_c), Antoine["isooctane"])
...
md"For n-heptane these parameters are: $nhep
	
For iso-octane these parameters are: $isooct
"
end

# ╔═╡ aaaf34c3-620b-40b3-bfbd-a0771f6de5df
md"Pressure (mmHg): $(@bind p_atmos Slider(600.0:5.0:1000.0, 760.0, true))"

# ╔═╡ d2714749-0e6a-49fc-9383-a4c692564198
begin
	temp_range = 90:.01:110
	# heptane_vp = map(t -> vapor(t, Antoine["nheptane"]), kelvin.(temp_range))
	# isooctane_vp = map(t -> vapor(t, isooctane), kelvin.(temp_range))
...
manifest_format = "2.0"
project_hash = "e6a3d4eb07b9ffb4c4bf519a4a5b76b0c646d243"
